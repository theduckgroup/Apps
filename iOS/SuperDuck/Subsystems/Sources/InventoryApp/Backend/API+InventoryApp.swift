...
extension API {
    ...
    func stockChangesMeta(storeId: String, userId: String) async throws -> [StockChangeMeta] {
        ...
        return try await get(path: "/inventory-app/stores/\(storeId)/stock/changes/meta/by-user/\(userId)")
    }
    ...
    func stockChange(storeId: String, changeId: String) async throws -> StockChange {
        ...
        return try await get(path: "/inventory-app/stores/\(storeId)/stock/changes/\(changeId)")
    }
}
