...
// import ConfirmDeleteModal, { ConfirmDeleteModalOptions } from 'src/app/InvStore-items/ConfirmDeleteModal'
...
export default function StoreEditorPage() {
  ...
  const { data: store, isLoading: isLoadingStore, error: storeError, refetch: fetchStore } = useQuery({
  ...
  const [items, setItems] = useState<InvStore.Item[]>([])
  const [sections, setSections] = useState<InvStore.Section[]>([])

  useEffect(() => {
    if (store) {
      setItems(store.catalog.items)
      setSections(store.catalog.sections)
    }
  }, [store])
  ...
  const { mutate: submit, error: submitError, isPending: isSubmitting } = useMutation({
    mutationFn: async () => {
      ...
      return (await axios.put(`store/${storeId}/catalog`, body)).data
    }
  })
  ...
  return (
    <Stack align='flex-start'>
      ...
      {
        isLoadingStore ? <Text>Loading...</Text> :
          storeError ? <Text c='red'>{formatError(storeError)}</Text> :
            <ItemList
              items={items}
              setItems={setItems}
              sections={sections}
